...
maximize: The book sells and revenue by identifying the frequently buyed books  
...
df=pd.read_csv('Association Rules/book.csv')
df

####################################
df.columns
'''
the dataset contains 11 columns named as :
...
'''
######################################
#find out if there are any null values in dataset
a=pd.isnull(df)
a.sum()
'''
ChildBks     0
...
data=pd.read_csv('Association Rules/book.csv')
data
